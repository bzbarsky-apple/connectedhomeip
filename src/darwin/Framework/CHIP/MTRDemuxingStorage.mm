...
{
    // For now, just store all the fabric table bits in memory only.
    // TODO: If we start supporting generating some of these pieces internally
    // instead of having them all be externally provided, or start supporting
    // starting up with previously stored values, we will need to start
    // persisting some of these things.
    //
